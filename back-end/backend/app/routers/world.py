...
@router.post("/games/{game_id}/background", response_model=BackgroundResponse)
async def generate_background(
    ...
    # 2. 呼叫 LLM 產生背景
    background_text = await call_llm_for_background(req.prompt)

    # 3. 存到 database
    mem.save_background(game_id, background_text)

    # 4. 回傳
    return BackgroundResponse(background=background_text)
